...
dpu_lo_ctr   = nan(29,1);
% ==> high contrast cases
dbias_lo_ctr_hcr = nan(29,1);
dpu_lo_ctr_hcr   = nan(29,1);
...
% ==> PF curve fits
PFs_predPF_dynr_lo_hcr = {};
PFs_predPF_dynr_hi_hcr = {};
...
%     stim_idx = ( abs(ori) > abs(or(2)) );
    ...
    % ==> cw context & lo contrast, and lo dynamic range
    cw_hcr_ldr_cng = (ctx == 1) & (ctr == max(ctr)) & (lo_dynr) & (cho == 1) & stim_idx;    
    % ==> cw context & lo contrast, and hi dynamic range
    cw_hcr_hdr_cng = (ctx == 1) & (ctr == max(ctr)) & (hi_dynr) & (cho == 1) & stim_idx;    
    % ==> ccw context (congruent choices)
    % ==> ccw context & lo contrast, and lo dynamic range
    ccw_hcr_ldr_cng = (ctx == -1) & (ctr == max(ctr)) & (lo_dynr) & (cho == -1) & stim_idx;    
    % ==> ccw context & lo contrast, and hi dynamic range
    ccw_hcr_hdr_cng = (ctx == -1) & (ctr == max(ctr)) & (hi_dynr) & (cho == -1) & stim_idx;      
    % ==> cw context (incongruent choices)
    % ==> cw context & lo contrast, and lo dynamic range
    cw_hcr_ldr_icg = (ctx == 1) & (ctr == max(ctr)) & (lo_dynr) & (cho == -1) & stim_idx;    
    % ==> cw context & lo contrast, and hi dynamic range
    cw_hcr_hdr_icg = (ctx == 1) & (ctr == max(ctr)) & (hi_dynr) & (cho == -1) & stim_idx;            
    % ==> ccw context (incongruent choices)
    % ==> ccw context & lo contrast, and lo dynamic range
    ccw_hcr_ldr_icg = (ctx == -1) & (ctr == max(ctr)) & (lo_dynr) & (cho == 1) & stim_idx;    
    % ==> ccw context & lo contrast, and hi dynamic range
    ccw_hcr_hdr_icg = (ctx == -1) & (ctr == max(ctr)) & (hi_dynr) & (cho == 1) & stim_idx;       
    ...
    for theta = 1:7
        ...
        % ==> low dynamic range split ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        % ==> cw congruent choice, lo ctr, lo dynr
        cho_cw_ct_cw_cr_hi_dynr_lo(theta)   = sum(cw_hcr_ldr_cng   & ori==or(theta));% / sum((ctx == 1) & (ctr == min(ctr)) & (lo_dynr)  & ori==or(theta));
        % ==> cw incongruent choice, lo ctr, lo dynr
        cho_cw_ct_ccw_cr_hi_dynr_lo(theta)  = sum(ccw_hcr_ldr_icg  & ori==or(theta));% / sum((ctx == -1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));        
        % ==> ccw incongruent choice, lo ctr, lo dynr
        cho_ccw_ct_cw_cr_hi_dynr_lo(theta)  = sum(cw_hcr_ldr_icg   & ori==or(theta));% / sum((ctx == 1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));
        % ==> ccw congruent choice, lo ctr, lo dynr
        cho_ccw_ct_ccw_cr_hi_dynr_lo(theta) = sum(ccw_hcr_ldr_cng  & ori==or(theta));% / sum((ctx == -1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));
                
        % ==> high dynamic range split ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
         % ==> cw congruent choice, lo ctr, lo dynr
        cho_cw_ct_cw_cr_hi_dynr_hi(theta)   = sum(cw_hcr_hdr_cng   & ori==or(theta));% / sum((ctx == 1) & (ctr == min(ctr)) & (lo_dynr)  & ori==or(theta));
        % ==> cw incongruent choice, lo ctr, lo dynr
        cho_cw_ct_ccw_cr_hi_dynr_hi(theta)  = sum(ccw_hcr_hdr_icg  & ori==or(theta));% / sum((ctx == -1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));        
        % ==> ccw incongruent choice, lo ctr, lo dynr
        cho_ccw_ct_cw_cr_hi_dynr_hi(theta)  = sum(cw_hcr_hdr_icg   & ori==or(theta));% / sum((ctx == 1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));
        % ==> ccw congruent choice, lo ctr, lo dynr
        cho_ccw_ct_ccw_cr_hi_dynr_hi(theta) = sum(ccw_hcr_hdr_cng  & ori==or(theta));% / sum((ctx == -1) & (ctr == min(ctr)) & (lo_dynr) & ori==or(theta));        
    ...
    % ==> counts
    % ==> context 1 ccw response (incongruent) & ==> context 2 ccw response (congruent)
    nccw_dynr_lo_hcr = [cho_ccw_ct_cw_cr_hi_dynr_lo; cho_ccw_ct_ccw_cr_hi_dynr_lo]; 
    % ==> context 1 cw response (congruent)    & ==> context 2 cw response (incongruent)
    ncw_dynr_lo_hcr  = [cho_cw_ct_cw_cr_hi_dynr_lo;  cho_cw_ct_ccw_cr_hi_dynr_lo]; 
    % ==> pass through PF function fit
    obFun    = @(paramVec) giveNLL(paramVec, or, nccw_dynr_lo_hcr, ncw_dynr_lo_hcr, 'M1');    
    % ==> run 
    paramEst_M1_dynr_lo_hcr = fmincon(obFun, startVec_M1, [], [], [], [], LB_M1, UB_M1, [], options);
    % ==> return pf
    [~, predPF_dynr_lo_hcr] = giveNLL(paramEst_M1_dynr_lo_hcr, or, nccw_dynr_lo_hcr, ncw_dynr_lo_hcr, 'M1');
    %  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    
    % ==> high dynamic range cases ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    % ==> counts 
    % ==> context 1 ccw response (incongruent) & ==> context 2 ccw response (congruent)
    nccw_dynr_hi_hcr = [cho_ccw_ct_cw_cr_hi_dynr_hi; cho_ccw_ct_ccw_cr_hi_dynr_hi]; 
    % ==> context 1 cw response (congruent)    & ==> context 2 cw response (incongruent)
    ncw_dynr_hi_hcr  = [cho_cw_ct_cw_cr_hi_dynr_hi;  cho_cw_ct_ccw_cr_hi_dynr_hi]; 
    % ==> pass through PF function fit
    obFun    = @(paramVec) giveNLL(paramVec, or, nccw_dynr_hi_hcr, ncw_dynr_hi_hcr, 'M1');    
    % ==> run 
    paramEst_M1_dynr_hi_hcr = fmincon(obFun, startVec_M1, [], [], [], [], LB_M1, UB_M1, [], options);
    % ==> return pf
    [~, predPF_dynr_hi_hcr] = giveNLL(paramEst_M1_dynr_hi_hcr, or, nccw_dynr_hi_hcr, ncw_dynr_hi_hcr, 'M1');
    %  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    figure(4);
    subplot(5,6,iSfln); title(['Session: ', num2str(iSfln)]);
    plot(or,predPF_dynr_lo_hcr(1,:),'g.-')
    hold on; hold all;
    plot(or,predPF_dynr_lo_hcr(2,:),'m.-')
    hold on; hold all;
    plot(or,predPF_dynr_hi_hcr(1,:),'g+:')
    hold on; hold all;
    plot(or,predPF_dynr_hi_hcr(2,:),'m+:')    
    xlabel('stimulus orientation')
    ylabel('Proportion cw PF fit')
    drawnow;    
    ...
    % ==> delta bias (this is a different computation from original)
    dbias_lo_ctr_hcr(iSfln) = (paramEst_M1_dynr_lo_hcr(5) - paramEst_M1_dynr_lo_hcr(4)) - (paramEst_M1_dynr_hi_hcr(5) - paramEst_M1_dynr_hi_hcr(4));
    %paramEst_M1_dynr_lo_hcr(5) - paramEst_M1_dynr_hi)_hcr(5);    
    % ==> delta perceptual uncertainty
    dpu_lo_ctr_hcr(iSfln)   = paramEst_M1_dynr_lo_hcr(3) - paramEst_M1_dynr_hi_hcr(3);    
    
    % ==> save PF curve fits
    PFs_predPF_dynr_lo{iSfln} = predPF_dynr_lo;
    PFs_predPF_dynr_hi{iSfln} = predPF_dynr_hi;  
    % ==> save PF curve fits
    PFs_predPF_dynr_lo_hcr{iSfln} = predPF_dynr_lo_hcr;
    PFs_predPF_dynr_hi_hcr{iSfln} = predPF_dynr_hi_hcr;      
    
end

% ==> dynamic range and choice predictivity & AIC delta, and LLR (low contrast trials)
...
% ==> choice predictivity and delta bias + delta uncertainty
[rb,pb] = corr(dvCatPerf(ppl), dbias_lo_ctr_hcr(ppl));
[rp,pp] = corr(dvCatPerf(ppl), dpu_lo_ctr_hcr(ppl));

% ==> Use AIC computation from F2 instead of choice predictivity
[rb_aic,pb_aic] = corr(aic_d(ppl), dbias_lo_ctr_hcr(ppl));
[rp_aic,pp_aic] = corr(aic_d(ppl), dpu_lo_ctr_hcr(ppl));

% ==> with LLR from F2
[rb_llr,pb_llr] = corr(llr(ppl), dbias_lo_ctr_hcr(ppl));
[rp_llr,pp_llr] = corr(llr(ppl), dpu_lo_ctr_hcr(ppl));


% ==> pearson correlation between delta bias and delta uncertainty
% ==> ommitting session 1 here!
[r_bp,p_bp] = corr(dbias_lo_ctr_hcr(ppl),dpu_lo_ctr_hcr(ppl));

figure(6); set(gcf,'color','white'); set(gcf,'Position',[723 4 1104 958]);
subplot(3,3,1);
scatter(dvCatPerf(ppl), dbias_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(dvCatPerf(ppl)+0.005, dbias_lo_ctr_hcr(ppl)-0.005, string([ppl]),'FontSize',6); 
title(['r = ',num2str(rb), ', p = ',num2str(pb)]);
xlabel('Choice Predictivity');
ylabel('\Delta bias');
subplot(3,3,2);
scatter(dvCatPerf(ppl), dpu_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(dvCatPerf(ppl)+0.005, dpu_lo_ctr_hcr(ppl)-0.005,string([ppl]),'FontSize',6);
title(['r = ',num2str(rp), ', p = ',num2str(pp)]);
xlabel('Choice Predictivity');
ylabel('\Delta perceptual uncertainty (slope)');
subplot(3,3,3);
scatter(dbias_lo_ctr_hcr(ppl), dpu_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(dbias_lo_ctr_hcr(ppl)+0.005,dpu_lo_ctr_hcr(ppl)-0.005,string([ppl]),'FontSize',6);
title(['r = ',num2str(r_bp), ', p = ',num2str(p_bp)]);
xlabel('\Delta bias')
ylabel('\Delta perceptual uncertainty (slope)')
subplot(3,3,4);
scatter(aic_d(ppl), dbias_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(aic_d(ppl)+0.005, dbias_lo_ctr_hcr(ppl)-0.005, string([ppl]),'FontSize',6); 
title(['r = ',num2str(rb_aic), ', p = ',num2str(pb_aic)]);
xlabel('AIC \Delta');
ylabel('\Delta bias');
subplot(3,3,5);
scatter(aic_d(ppl), dpu_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(aic_d(ppl)+0.005, dpu_lo_ctr_hcr(ppl)-0.005,string([ppl]),'FontSize',6);
title(['r = ',num2str(rp_aic), ', p = ',num2str(pp_aic)]);
xlabel('AIC \Delta');
ylabel('\Delta perceptual uncertainty (slope)');
subplot(3,3,7);
scatter(llr(ppl), dbias_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(llr(ppl)+0.005, dbias_lo_ctr_hcr(ppl)-0.005, string([ppl]),'FontSize',6); 
title(['r = ',num2str(rb_llr), ', p = ',num2str(pb_llr)]);
xlabel('Log-likelihood ratio');
ylabel('\Delta bias');
subplot(3,3,8);
scatter(llr(ppl), dpu_lo_ctr_hcr(ppl),30,'ko','filled'); hold on; hold all;
text(llr(ppl)+0.005, dpu_lo_ctr_hcr(ppl)-0.005,string([ppl]),'FontSize',6);
title(['r = ',num2str(rp_llr), ', p = ',num2str(pp_llr)]);
xlabel('Log-likelihood ratio');
ylabel('\Delta perceptual uncertainty (slope)');
