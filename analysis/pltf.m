function pltf(db, dp, aicd, llr, chpd, ppl, fg, ctrs, ids)

% ==> marker colors
clr = [repmat([0.15,0.75,0.5],13,1); repmat([1,0.75,0],16,1)];
...
for i = ppl
    ...
    hold on; hold all;
    text(chpd(i)+0.005, db(i)-0.005, string([ids(i)]),'FontSize',6); 
end
...
for i = ppl
    ...
    hold on; hold all;
    text(chpd(i)+0.005, dp(i)-0.005,string([ids(i)]),'FontSize',6);
end
...
for i = ppl
    ...
    hold on; hold all;
    text(db(i)+0.005,dp(i)-0.005,string([ids(i)]),'FontSize',6);
end
...
for i = ppl
    ...
    hold on; hold all;
    text(aicd(i)+0.005, db(i)-0.005, string([ids(i)]),'FontSize',6); 
end
...
for i = ppl
    ...
    hold on; hold all;
    text(aicd(i)+0.005, dp(i)-0.005,string([ids(i)]),'FontSize',6);
end
...
for i = ppl
    ...
    hold on; hold all;
    text(llr(i)+0.005, db(i)-0.005, string([ids(i)]),'FontSize',6); 
end
...
for i = ppl
    ...
    hold on; hold all;
    text(llr(i)+0.005, dp(i)-0.005,string([ids(i)]),'FontSize',6);
end
